...
# ─── finalize_cancel_turn ───────────────────────────────────


class TestFinalizeCancelTurn:
    """finalize_cancel_turn: 取消 active turn 的历史清理。"""
    ...
    async def test_marks_init_items_as_cancelled(self):
        ...
        with patch(_MOCK_UPDATE, AsyncMock()), \
             patch(_MOCK_APPEND, AsyncMock(side_effect=lambda item: item)):
            await history.finalize_cancel_turn()
    ...
    @pytest.mark.asyncio
    async def test_supplements_missing_tool_results(self):
        """场景 B-2：ASSISTANT 声明了 tool_call 但无对应 TOOL 记录时，应补写 CANCELLED TOOL 记录。"""
        ...
        appended_items: list[GtAgentHistory] = []
        ...
        with patch(_MOCK_APPEND, AsyncMock(side_effect=_track_append)), \
             patch(_MOCK_UPDATE, AsyncMock()):
            await history.finalize_cancel_turn()
    ...
    @pytest.mark.asyncio
    async def test_init_assistant_with_tool_calls(self):
        """场景 B-3：ASSISTANT INIT 占位项带 tool_calls，应标记 CANCELLED。"""
    ...
    @pytest.mark.asyncio
    async def test_cancelled_items_excluded_from_infer_messages(self):
        """CANCELLED 占位项在 build_infer_messages 时应被跳过。"""
        init_assistant = GtAgentHistory.build_placeholder(role=OpenaiApiRole.ASSISTANT, status=AgentHistoryStatus.INIT)
        init_assistant.agent_id = 1
