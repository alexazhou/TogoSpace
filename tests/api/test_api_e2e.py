...
async def test_get_room_messages(http_client: aiohttp.ClientSession, backend_base: str):
    # room_id is now room@team format
    async with http_client.get(f"{backend_base}/rooms/general@e2e/messages") as resp:
        assert resp.status == 200
        data = await resp.json()
...
async def test_room_not_found(http_client: aiohttp.ClientSession, backend_base: str):
    async with http_client.get(f"{backend_base}/rooms/nonexistent@noexist/messages") as resp:
