...
class TestSchedulerRun(ServiceTestCase):
    ...
    async def test_scheduler_runs_agent_on_turn_event(self):
        ...
        with patch("service.schedulerService.agentService.get_agent", return_value=alice):
            ...
            msg = Message(
                ...
                payload={"agent_name": "alice", "room_id": room.room_id, "room_name": "r1", "room_key": f"r1@{TEAM}", "team_name": TEAM},
            )
    ...
    async def test_different_rooms_not_deduplicated(self):
        ...
        with patch("service.schedulerService.agentService.get_agent", return_value=alice):
            msg_r1 = Message(
                topic=MessageBusTopic.ROOM_AGENT_TURN,
                payload={"agent_name": "alice", "room_id": r1.room_id, "room_name": "r1", "room_key": f"r1@{TEAM}", "team_name": TEAM},
            )
            msg_r2 = Message(
                topic=MessageBusTopic.ROOM_AGENT_TURN,
                payload={"agent_name": "alice", "room_id": r2.room_id, "room_name": "r2", "room_key": f"r2@{TEAM}", "team_name": TEAM},
            )
