...
```json
{
  ...
  "members": [
    {
      "name": "alice", "agent": "executive_agent", "model": "claude-opus-4-6",
      "driver": { "type": "native" }
    },
    {
      "name": "bob", "agent": "engineer_agent", "model": "claude-sonnet-4-6",
      "driver": { "type": "claude_sdk", "allowed_tools": ["Read", "Write", "Edit", "Bash"] }
    },
    {
      "name": "carol", "agent": "backend_dev", "model": "claude-sonnet-4-6",
      "driver": { "type": "claude_sdk", "allowed_tools": ["Read", "Write", "Edit", "Bash"] }
    },
    {
      "name": "dave", "agent": "frontend_dev", "model": "claude-haiku-4-5",
      "driver": { "type": "native" }
    },
    {
      "name": "eve", "agent": "finance_agent", "model": "claude-sonnet-4-6",
      "driver": { "type": "native" }
    },
    {
      "name": "frank", "agent": "accountant", "model": "claude-haiku-4-5",
      "driver": { "type": "tsp" }
    }
  ]
}
...
- [ ] `members` 列表存储每个人的 `agent`（模板）、`model` 和 `driver`（含 `type` 及驱动专属选项）配置，与组织归属解耦。
...
- `members`：成员运行配置列表，每条记录包含 `name`（成员名）、`agent`（Agent 模板名）、`model`（使用的模型）和 `driver`（驱动配置对象，含 `type` 字段及驱动专属选项，如 `claude_sdk` 需配置 `allowed_tools`）。成员的部门归属由 `dept_tree` 定义，此处不重复声明。

### API 操作

| 操作 | 说明 |
|------|------|
| `GET /teams/{team}/dept_tree` | 获取当前部门树结构 |
| `PUT /teams/{team}/dept_tree/{dept}/manager` | 变更某部门的主管 |
| `POST /teams/{team}/dept_tree/{dept}/members` | 将成员加入某部门（可指定是否担任主管） |
| `DELETE /teams/{team}/dept_tree/{dept}/members/{member}` | 将成员从部门移除（进入 idle）；若为主管，请求体中须指定继任主管 |
| `GET /teams/{team}/members?status=idle` | 查询所有休闲成员 |
