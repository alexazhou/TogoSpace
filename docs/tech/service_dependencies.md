...
graph TD
    ...
    route --> controller
    controller --> roleTemplateService
    ...
    agentService -->|load role templates| roleTemplateService
...
| `route / controller` | Web API 层，处理 HTTP 请求与 WebSocket 推送，查询 Agent/Room/Team/Dept 状态 | roleTemplateService / agentService / roomService / teamService / deptService |
| `schedulerService` | 任务生命周期管理，监听轮次事件并激活 Agent 内部任务协程 | agentService (`Agent.consume_task`) / roomService / messageBus |
| `roleTemplateService` | 角色模板配置管理，维护 RoleTemplate 定义与 prompt 加载 | 无 |
| `agentService` | **[自治核心]** 维护 Agent 实例及其任务队列，执行对话轮次与 Tool 调用，自主维护活跃状态 | roleTemplateService / llmService / roomService / funcToolService / persistenceService / messageBus |
...
| `deptService` | 部门树与成员归属管理（V10） | ormService（通过 DAL） |
