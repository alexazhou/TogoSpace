...
class AgentDriverHost(Protocol):
    ...
    async def _infer(self, tools) -> OpenAIMessage: ...
    async def _execute_tool(self) -> None: ...
```

Driver 通过 `self.host` 访问以上字段和方法。当前 host 是 Agent 实例。
...
关键发现：
- `host.current_db_task` **没有任何 driver 直接使用**（仅 `_execute_tool()` 内部从 self 读取）
- `host._infer()` **没有任何 driver 调用**
...
TurnRunner 需要新增：
- `current_db_task` 属性（从 Consumer 获取，或改为不需要）
- 保留 `_infer()` 和 `_execute_tool()` 方法（已有）

**关于 current_db_task**：
- Protocol 要求 `current_db_task: GtAgentTask`
- 仅 `_execute_tool()` 使用，用于获取 room_id
- `_execute_tool()` 仅被 claudeSdkDriver 调用
- 建议修改：`_execute_tool()` 不再从 current_db_task 读取，而是维护一个 `_current_room` 上下文（在 `run_chat_turn` 开始时设置，结束时清除）
- 这样可以从 Protocol 中移除 `current_db_task`
...
Agent 构造函数简化为：
```python
def __init__(self, gt_agent, system_prompt, driver_config=None,
             agent_workdir="", max_function_calls=5):
    self.gt_agent = gt_agent
...
变化：移除 `current_db_task`（改用 TurnRunner 内部的 `_current_room` 上下文）。
