...
优化房间消息的展示体验，使 Agent 在处理当前房间任务时，用户能够直观看到“正在输入”的过程，并在结束后看到该占位内容自然变为最终消息：

- **输入中可见**：当 Agent 正在处理当前房间的发言任务时，聊天窗口中会出现明确的“输入中”占位提示，而不是长时间无反馈
- **原地变为结果**：Agent 完成当前轮发言后，占位内容直接变成最终消息，避免重复插入一条新的消息造成视觉跳动
- **顺序自然稳定**：在 Agent 输入期间，后续进入房间的消息仍保持自然时序，占位与最终消息的展示位置一致，不出现消息顺序错乱
- **异常状态可感知**：如果 Agent 在处理过程中失败、取消或最终未产生有效发言，界面中的占位状态应被正确清理，避免残留“正在输入”的假状态
- **实时性增强**：Web / TUI 前端都能实时感知 Agent 的发言过程，降低用户对系统“卡住”或“无响应”的误判
- **体验统一**：该能力应与现有房间消息流、未读计数和实时推送机制保持一致，不破坏现有聊天使用习惯
