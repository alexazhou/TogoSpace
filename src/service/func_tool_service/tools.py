...
def get_agent_list(_context: ChatContext = None) -> List[str]:
    ...
    """
    logger.\1(f"get_agent_list: 获取 agent 列表")
    if _context is None:
        return []
    senders = []
    seen = set()
...
def create_chat(room_name: str, _context: ChatContext = None) -> str:
    ...
    """
    logger.\1(f"create_chat: 切换到聊天室 {room_name}")
    if _context is None:
...
def send_chat_msg(chat_windows_name: str, msg: str, _context: ChatContext = None) -> str:
    ...
    Returns:
        成功返回 "success"
    """
    logger.\1(f"send_chat_msg: 向 {chat_windows_name} 发送消息: {msg}")
    ...
    if _context is not None:
        try:
            ...
            logger.\1(f"send_chat_msg: 聊天室 '{chat_windows_name}' 不存在，消息已忽略")
    else:
        logger.\1("send_chat_msg: 聊天室上下文未设置")
